...
CD_single <- function(file.now, file.old, Min = 0.2, Max = 1, predictions = NULL,
  ...
  method_diff_pos <- function(now, old, Min, Max) {
    # Image difference. Positives Only
    dif <- now
    dif[] <- now[] - old[]
    # Keep positve changes only (now > old). Darker Parts will be classified in next iteration.
    hum <- dif
    hum[which(dif < 0)] <- 0
    # Classify significant changes with tresholds.
...
CD_list <- function(img.list, ...) {
  ...
  act.Data <- foreach::foreach(
    now = img.list[1:new.stop],
